...
class WialonShiftSyncCheckpointTest extends TestCase
{
    ...
    public function test_planner_does_not_duplicate_items_and_skips_completed_without_force(): void
    {
        ...
        $unit = $this->equipment('Unit A', '7001', $group);
        ...
        EquipmentDailyStat::query()->create([
            ...
            'overtime_hours' => 2,
            'total_hours' => 8,
            'calculation_source' => FleetShiftDailyStatsSyncService::SOURCE,
    ...
    public function test_planner_reopens_completed_shift_item_when_saved_rows_are_still_unknown(): void
    {
        $group = $this->group('601701912');
        $unit = $this->equipment('Unit A', '7001', $group);
        $item = $this->checkpoint($group, WialonReportSyncItem::STATUS_COMPLETED);
        $item->forceFill([
            'attempts' => 2,
            'rows_received' => 1,
            'rows_saved' => 1,
        ...
        EquipmentDailyStat::query()->create([
            ...
            'worked_hours' => 0,
            'calculation_source' => FleetShiftDailyStatsSyncService::SOURCE,
            'calculation_status' => 'shift_unknown',
    ...
    public function test_planner_keeps_completed_shift_item_when_it_has_known_shift_rows(): void
    {
        $group = $this->group('601701913');
        $unit = $this->equipment('Unit A', '7001', $group);
        $item = $this->checkpoint($group, WialonReportSyncItem::STATUS_COMPLETED);
        $item->forceFill([
            'attempts' => 2,
        ...
        $this->assertSame(WialonReportSyncItem::STATUS_COMPLETED, $item->status);
        $this->assertSame(2, $item->attempts);
        $this->assertSame(1, $item->rows_saved);
    }
