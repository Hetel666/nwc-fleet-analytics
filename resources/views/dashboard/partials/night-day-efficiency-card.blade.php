...
<section class="panel p-3 dashboard-card dashboard-work-status-card d-flex flex-column">
                ...
                <table class="table table-sm align-middle mb-0">
                    ...
                    @foreach ($statuses as $status)
                        <tr
                            ...
                            role="button"
                            tabindex="0"
                            data-drilldown-title="Gün daxilində gecə effektivliyi — {{ $categoryLabels[$status] }}"
                            data-drilldown-ownership="{{ $drilldownOwnership }}"
                            data-drilldown-mode="night_day_efficiency_projects"
                            ...
                            data-drilldown-endpoint-url="{{ route('api.dashboard.night-day-efficiency.projects') }}"
                            data-drilldown-units-endpoint-url="{{ route('api.dashboard.night-day-efficiency.units') }}"
                            data-drilldown-export-url="{{ route('api.dashboard.night-day-efficiency.export') }}"
    ...
    @else
        <div class="dashboard-empty flex-grow-1">Seçilmiş dövr üçün gün daxilində gecə effektivliyi məlumatı yoxdur</div>
    @endif
</section>
