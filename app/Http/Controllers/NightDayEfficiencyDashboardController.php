...
class NightDayEfficiencyDashboardController extends Controller
{
    public function summary(
        ...
        return ['data' => $displayConfiguration->filterSummaryRows($dashboard->summary($filters), 'night_day_efficiency')];
    }
    ...
    public function projects(
        ...
        abort_unless($dashboard->isReady(), 503, 'Night day efficiency storage is not ready.');
        $rows = $dashboard->paginateProjects($this->visibleFilters($request, $displayConfiguration));

        return [
            'title' => 'Gün daxilində gecə effektivliyi - '.($request->string('status')->toString() ?: 'Layihələr'),
            'columns' => [
                'project' => 'Layihə',
                'ownership' => 'Ownership',
                'status' => 'Status',
                ...
                'synced_days_count' => 'Sinxronlaşdırılmış gün sayı',
            ],
            ...$this->paginated($rows),
    ...
    public function units(
        ...
        return [
            'title' => 'Gün daxilində gecə effektivliyi - Texnika siyahısı',
            'columns' => [
                'name' => 'Texnika',
                'project' => 'Layihə',
                'vehicle_type' => 'Texnika növü',
                'ownership' => 'Ownership',
                'period' => 'Seçilmiş dövr',
                'synced_days_count' => 'Sinxronlaşdırılmış gün sayı',
                'total_engine_hours' => 'Ümumi Engine hours',
                'average_engine_hours' => 'Orta Engine hours',
                'started_at' => 'Faktiki başlama',
                'ended_at' => 'Faktiki bitmə',
                'mileage' => 'Yürüş',
                'status_label' => 'Yekun status',
            ],
            ...$this->paginated($rows),
            'summary' => ['total' => $rows->total()],
        ];
    ...
    private function visibleFilters(Request $request, DashboardDisplayConfigurationService $displayConfiguration): array
    {
        ...
        return $displayConfiguration->applyVisibleStatusesToFilters($filters, 'night_day_efficiency');
    }
}
