...
class ForeignProjectGeofenceMonitoringService
{
    ...
    public function processUnitPosition(Equipment $unit, ?array $position): ?UnitForeignGeofenceInterval
    {
        ...
        return DB::transaction(function () use ($unit, $position, $analysis): ?UnitForeignGeofenceInterval {
            $lockedUnit = Equipment::query()
                ->with(['type', 'project', 'projectWialonGroup'])
                ->whereKey($unit->id)
                ->lockForUpdate()
                ->firstOrFail();
            $openIntervals = $this->openIntervals($lockedUnit);
            $openInterval = $openIntervals->first();
            $positionAt = $analysis['position_at'];

            if (! $analysis['is_foreign_project_geofence']) {
                $this->closeForeignGeofenceIntervals($openIntervals, $position, $positionAt);
            ...
            if (
                ...
                $openInterval->update([
                    ...$this->intervalMetadata($lockedUnit, $homeGeofence, $currentGeofence, $position),
            ...
            return $this->openForeignGeofenceInterval($lockedUnit, $homeGeofence, $currentGeofence, $position, $positionAt, $openIntervals);
        });
    }
    ...
    public function openForeignGeofenceInterval(
        ...
        array $position,
        Carbon $enteredAt,
        ?Collection $openIntervals = null
    ): UnitForeignGeofenceInterval {
        $this->closeForeignGeofenceIntervals($openIntervals ?? $this->openIntervals($unit), $position, $enteredAt);

        return UnitForeignGeofenceInterval::query()->create([
            ...$this->intervalMetadata($unit, $homeGeofence, $foreignGeofence, $position),
            'unit_id' => $unit->id,
     ...
     */
    private function intervalMetadata(Equipment $unit, ?Geofence $homeGeofence, Geofence $foreignGeofence, array $position): array
    {
        $unit->loadMissing(['project', 'projectWialonGroup']);
        $homeGeofences = $this->homeProjectGeofences($unit);

        return [
            'wialon_unit_id' => $unit->wialon_unit_id,
            'source_group_id' => $unit->projectWialonGroup?->wialon_group_id ?? $unit->matched_wialon_group_id,
            'source_group_name' => $unit->projectWialonGroup?->name ?? $unit->matched_wialon_group_name,
            'source_group_ids_json' => collect([$unit->projectWialonGroup?->wialon_group_id, $unit->matched_wialon_group_id])
                ->filter()
                ->unique()
                ->values()
                ->all(),
            ...
            'home_project_name' => $unit->project?->name,
            'home_geofence_id' => $homeGeofence?->id,
            'home_geofence_ids_json' => $homeGeofences->pluck('id')->values()->all(),
            'home_geofence_names_json' => $homeGeofences->pluck('name')->values()->all(),
            'foreign_project_id' => $foreignGeofence->project_id,
            'foreign_project_name' => $foreignGeofence->project?->name,
            'foreign_geofence_id' => $foreignGeofence->id,
            'foreign_geofence_name' => $foreignGeofence->name,
            'match_method' => $foreignGeofence->wialon_geofence_id ? 'wialon_geofence_id' : 'geometry',
            'match_status' => 'matched',
            'reason' => 'inside_foreign_project_geofence',
            'source' => 'local_position',
            'calculated_at' => $this->positionTimestamp($position),
        ];
    }
