...
class WialonDaytimeEfficiencyReportService
{
    ...
    public function settings(): array
    {
        ...
        $resourceId = (int) config('fleet.wialon.daytime_efficiency_report_resource_id');
        $templateId = (int) config('fleet.wialon.daytime_efficiency_report_template_id');
        $templateName = (string) config('fleet.wialon.daytime_efficiency_report_template_name');
        ...
        if ($resourceId <= 0 || $templateId <= 0 || $template === null) {
            throw new RuntimeException("Wialon daytime efficiency report '{$templateName}' was not found.");
        }

        if (($template['type'] ?? null) !== 'avl_unit_group') {
            throw new RuntimeException("Wialon daytime efficiency report '{$templateName}' is not bound to unit groups.");
        }
        ...
        return $this->resolvedSettings = [
            ...
            'chunk_size' => max(1, (int) config('fleet.wialon.daytime_efficiency_report_chunk_size', 500)),
            'timeout' => max(5, (int) config('fleet.wialon.daytime_efficiency_report_timeout', 90)),
        ];
    }
        ...
        return $this->reportSessionLock->run(function () use ($group, $from, $to, $sid): array {
            ...
            try {
                ...
                if (! is_array($reportResult) || ! is_array($reportResult['tables'] ?? null)) {
                    throw new RuntimeException('Wialon daytime efficiency report returned an invalid result structure.');
                }
                ...
                foreach ($reportResult['tables'] as $index => $table) {
                    ...
                    if (count($rows) !== $rowCount) {
                        throw new RuntimeException("Wialon daytime efficiency table {$index} returned ".count($rows)." of {$rowCount} rows.");
                    }
                ...
                if ($reportResult['tables'] !== []
                    ...
                    throw new RuntimeException('Wialon daytime efficiency report did not return the Engine hours table.');
                }
