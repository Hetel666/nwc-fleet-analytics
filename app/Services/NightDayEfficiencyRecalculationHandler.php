...
class NightDayEfficiencyRecalculationHandler
{
    public function __construct(
        private WialonService $wialon,
        private WialonNightDayEfficiencyReportService $reports,
        private WialonNightDayEfficiencyReportParser $parser,
    ...
    private function synchronizeProjectDate(
        NightDayEfficiencySyncRun $run,
        NightDayEfficiencySyncTask $task,
        ...
        if ($groups->isEmpty()) {
            throw new RuntimeException('The project has no active NWC or Icare Wialon groups.');
        }
        ...
        DB::transaction(function () use ($task, $date, $facts, $unmatched, $force): void {
            ...
            $stale = NightDayEfficiencyDailyFact::query()
                ->where('project_id', $task->project_id)
                ->whereDate('business_date', $date->toDateString());
            ...
            if ($facts !== []) {
                if ($force) {
                    NightDayEfficiencyDailyFact::query()->insert(array_values($facts));
                } else {
                    NightDayEfficiencyDailyFact::query()->upsert(
                        array_values($facts),
            ...
            DB::table('night_day_efficiency_unmatched_rows')->where('task_id', $task->id)->delete();

            if ($unmatched !== []) {
                DB::table('night_day_efficiency_unmatched_rows')->insert(
    ...
    private function syncRun(HistoricalRecalculation $historicalRun): NightDayEfficiencySyncRun
    {
        $run = NightDayEfficiencySyncRun::query()->updateOrCreate(
        ...
        foreach ($historicalRun->tasks()->where('operation', HistoricalRecalculation::OPERATION_FETCH)->get() as $task) {
            $exists = NightDayEfficiencySyncTask::query()
                ->where('run_id', $run->id)
                ->where('project_id', $task->project_id)
            ...
            if (! $exists) {
                NightDayEfficiencySyncTask::query()->create([
                    'run_id' => $run->id,
                    'project_id' => $task->project_id,
    ...
    private function refreshRun(NightDayEfficiencySyncRun $run, ?string $error = null): void
    {
