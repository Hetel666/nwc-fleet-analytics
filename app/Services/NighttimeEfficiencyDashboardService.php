...
class NighttimeEfficiencyDashboardService
{
    public function __construct(private DashboardDateRangePolicy $dateRangePolicy) {}

    public function isReady(): bool
    {
        return Schema::hasTable('nighttime_efficiency_daily_facts');
    }
    ...
    public function projectRowsByOwnership(array $filters): array
    {
        ...
        $rows = $this->baseQuery($filters)
            ->join('projects', 'projects.id', '=', 'nighttime_efficiency_daily_facts.project_id')
            ->select('projects.id', 'projects.name', 'nighttime_efficiency_daily_facts.ownership')
            ->selectRaw($this->statusCountSql())
            ->selectRaw('COUNT(*) total')
            ->groupBy('projects.id', 'projects.name', 'nighttime_efficiency_daily_facts.ownership')
            ->orderByDesc('total')
            ->get();
    ...
    public function paginateProjects(array $filters): LengthAwarePaginator
    {
        ...
        $query = $this->baseQuery($filters)
            ->join('projects', 'projects.id', '=', 'nighttime_efficiency_daily_facts.project_id')
            ->select('projects.id as project_id', 'projects.name as project', 'nighttime_efficiency_daily_facts.ownership')
            ->selectRaw('nighttime_efficiency_daily_facts.efficiency_status as status')
            ->selectRaw('COUNT(*) as equipment_shifts_count')
            ->selectRaw('COUNT(DISTINCT nighttime_efficiency_daily_facts.wialon_unit_id) as unique_units_count')
            ->selectRaw('ROUND(AVG(nighttime_efficiency_daily_facts.engine_hours_decimal), 2) as average_engine_hours')
            ->groupBy('projects.id', 'projects.name', 'nighttime_efficiency_daily_facts.ownership', 'nighttime_efficiency_daily_facts.efficiency_status');

        return $query->orderByDesc('equipment_shifts_count')->orderBy('projects.name')
            ...
            ->through(fn (object $row): array => [
                ...
                'equipment_shifts_count' => (int) $row->equipment_shifts_count,
    ...
    public function paginateUnits(array $filters): LengthAwarePaginator
    {
        ...
        $sorts = [
            'date' => 'nighttime_efficiency_daily_facts.shift_date',
            'name' => 'nighttime_efficiency_daily_facts.unit_name',
            'project' => 'projects.name',
            'vehicle_type' => 'nighttime_efficiency_daily_facts.vehicle_type',
            'ownership' => 'nighttime_efficiency_daily_facts.ownership',
            'engine_hours' => 'nighttime_efficiency_daily_facts.engine_seconds',
            'mileage' => 'nighttime_efficiency_daily_facts.mileage_km',
            'status' => 'nighttime_efficiency_daily_facts.efficiency_status',
        ];
        $sort = $sorts[$filters['sort']] ?? $sorts['date'];

        return $query->orderBy($sort, $filters['direction'])->orderBy('nighttime_efficiency_daily_facts.unit_name')
            ->paginate($filters['per_page'], ['*'], 'page', $filters['page'])
    ...
    public function exportRows(array $filters): array
    {
        ...
        return $this->detailQuery($filters)
            ->orderBy('nighttime_efficiency_daily_facts.shift_date')
            ->orderBy('nighttime_efficiency_daily_facts.unit_name')
            ->get()
    ...
    public function export(array $filters): array
    {
        ...
        $filterRows = [
            ...
            ['Hesablama vahidi', 'Texnika-növbə'],
            ['İş vaxtı', '18:00-07:59 (növbə başladığı tarixə aiddir)'],
            ['Mənbə', 'night report Engine hours (api)'],
        ];
        $sections = [
            ['title' => 'Xülasə', 'columns' => ['Status', 'Texnika-növbə sayı', 'Unikal texnika sayı', 'Orta Engine hours'], 'rows' => $summaryRows],
            ['title' => 'Gecə detalları', 'columns' => ['Növbə tarixi', 'Texnika', 'Layihə', 'Ownership', 'Texnika növü', 'Engine hours', 'Növbə başlanğıcı', 'Növbə sonu', 'Faktiki başlanğıc', 'Faktiki son', 'Yürüş', 'Status'], 'rows' => $detailRows],
        ];

        return [
            'filename' => 'effektivlik-gece-'.$filters['from'].'-'.$filters['to'].'.xlsx',
            'title' => 'Gecə növbəsi üzrə effektivlik',
            'filters' => $filterRows,
            'sections' => $sections,
            'sheets' => [
                ['name' => 'Xülasə', 'title' => 'Gecə növbəsi üzrə effektivlik', 'filters' => $filterRows, 'sections' => [$sections[0]]],
                ['name' => 'Gecə detalları', 'title' => 'Gecə növbəsi üzrə effektivlik', 'filters' => $filterRows, 'sections' => [$sections[1]]],
            ],
        ];
    }
    ...
    private function baseQuery(array $filters): Builder
    {
        ...
        return DB::table('nighttime_efficiency_daily_facts')
            ->whereDate('shift_date', '>=', $filters['from'])
            ->whereDate('shift_date', '<=', $filters['to'])
    ...
    private function detailQuery(array $filters): Builder
    {
        return $this->baseQuery($filters)
            ->join('projects', 'projects.id', '=', 'nighttime_efficiency_daily_facts.project_id')
            ->select('nighttime_efficiency_daily_facts.*', 'projects.name as project');
    }
