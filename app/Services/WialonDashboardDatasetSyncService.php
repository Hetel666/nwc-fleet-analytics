...
class WialonDashboardDatasetSyncService
{
    ...
    public function syncDailyEngineHoursReport(array $filters, bool $force = false): array
    {
        ...
        $group = ProjectWialonGroup::query()
            ...
            ->where('ownership_type', $filters['ownership_type'])
    ...
    private function syncDailyGroup(array $filters, object $group, bool $force, bool $rootGroup): array
    {
        ...
        $lockKey = 'lock:'.$syncKey;

        return Cache::lock($lockKey, max(30, (int) config('dashboard.sync.lock_seconds', 300)))
            ->block(0, function () use ($filters, $group, $force, $rootGroup, $settings, $syncKey): array {
                return $this->syncDailyGroupWithLock($filters, $group, $force, $rootGroup, $settings, $syncKey);
            });
    }

    private function syncDailyGroupWithLock(array $filters, object $group, bool $force, bool $rootGroup, array $settings, string $syncKey): array
    {
        ...
        $equipmentIds = $equipment->pluck('id')->map(fn ($id): int => (int) $id)->all();
        ...
        $changed = DB::transaction(function () use ($reportedEquipmentIds, $equipmentById, $reportData, $date, $equipmentIds, $syncKey): bool {
            $deletedStats = 0;
            $deletedAggregates = 0;

            if ($equipmentIds !== []) {
                $deletedStats = DB::table('equipment_daily_stats')
                    ->where('stat_date', $date)
                    ->whereIn('equipment_id', $equipmentIds)
                    ->where('calculation_source', 'wialon_engine_hours_report')
                    ->delete();

                $deletedAggregates = DB::table('daily_unit_aggregates')
                    ->where('date', $date)
                    ->whereIn('equipment_id', $equipmentIds)
                    ->delete();
            }
            ...
            foreach ($reportedEquipmentIds as $equipmentId) {
                ...
                $utilization = $item->planned_daily_hours > 0
                    ...
                    : 0;
                $outsideGeofenceMinutes = (float) DB::table('equipment_daily_stats')
                    ->where('stat_date', $date)
                    ->where('equipment_id', $equipmentId)
                    ->value('outside_geofence_minutes');

                DB::table('equipment_daily_stats')->updateOrInsert(
                ...
                DB::table('daily_unit_aggregates')->updateOrInsert(
                    ...
                    [
                        ...
                        'mileage' => $distanceKm,
                        'geofence_outside_hours' => round($outsideGeofenceMinutes / 60, 2),
                        'created_at' => now(),
                        'updated_at' => now(),
                    ]
                );
            ...
            return $deletedStats > 0 || $deletedAggregates > 0 || count($reportedEquipmentIds) > 0;
        });

        $version = $changed ? $this->dataVersion->bump() : $this->dataVersion->current();
    ...
    private function rootOwnershipWialonGroup(string $ownershipType): ?object
    {
        ...
        ])->filter(fn (object $group): bool => trim($group->wialon_group_id) !== '')->get($ownershipType);
    }
    ...
    private function wialonReportTableByKind(array $tables, string $kind, bool $strict = false): ?array
    {
        ...
        return $strict ? null : ($tables[0] ?? null);
    }
