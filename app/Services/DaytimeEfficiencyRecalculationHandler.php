...
use App\Models\DaytimeEfficiencySyncRun;
use App\Models\DaytimeEfficiencySyncTask;
...
class DaytimeEfficiencyRecalculationHandler
{
    ...
    public function execute(HistoricalRecalculation $historicalRun, HistoricalRecalculationTask $historicalTask): int
    {
        ...
        $lock = Cache::lock(
            'daytime-efficiency:'.$historicalTask->project_id.':'.$date->toDateString(),
            (int) config('fleet.wialon.daytime_efficiency_sync_lock_seconds', 1800),
        );

        if (! $lock->get()) {
            throw new RuntimeException('Daytime efficiency synchronization is already running for this project and date.');
        }

        $run = $this->syncRun($historicalRun);
        $task = DaytimeEfficiencySyncTask::query()
            ->where('run_id', $run->id)
            ->where('project_id', $historicalTask->project_id)
            ->whereDate('business_date', $date->toDateString())
            ->first() ?? new DaytimeEfficiencySyncTask([
                'run_id' => $run->id,
    ...
    private function synchronizeProjectDate(DaytimeEfficiencySyncRun $run, DaytimeEfficiencySyncTask $task, CarbonImmutable $date): array
    {
        ...
        DB::transaction(function () use ($task, $date, $facts, $unmatched): void {
            ...
            if ($unmatched !== []) {
                DB::table('daytime_efficiency_unmatched_rows')->insert(
    ...
    private function syncRun(HistoricalRecalculation $historicalRun): DaytimeEfficiencySyncRun
    {
        $run = DaytimeEfficiencySyncRun::query()->updateOrCreate(
        ...
        foreach ($historicalRun->tasks()->where('operation', HistoricalRecalculation::OPERATION_FETCH)->get() as $task) {
            $exists = DaytimeEfficiencySyncTask::query()
                ->where('run_id', $run->id)
                ->where('project_id', $task->project_id)
            ...
            if (! $exists) {
                DaytimeEfficiencySyncTask::query()->create([
                    'run_id' => $run->id,
                    'project_id' => $task->project_id,
    ...
    private function refreshRun(DaytimeEfficiencySyncRun $run, ?string $error = null): void
    {
