...
class WialonNighttimeEfficiencyReportService
{
    ...
    public function settings(): array
    {
        ...
        $resourceId = (int) config('fleet.wialon.nighttime_efficiency_report_resource_id');
        $templateId = (int) config('fleet.wialon.nighttime_efficiency_report_template_id');
        $templateName = (string) config('fleet.wialon.nighttime_efficiency_report_template_name');
        ...
        if ($resourceId <= 0 || $templateId <= 0 || $template === null) {
            throw new RuntimeException("Wialon nighttime efficiency report '{$templateName}' was not found.");
        }

        if (($template['type'] ?? null) !== 'avl_unit_group') {
            throw new RuntimeException("Wialon nighttime efficiency report '{$templateName}' is not bound to unit groups.");
        }
        ...
        return $this->resolvedSettings = [
            ...
            'chunk_size' => max(1, (int) config('fleet.wialon.nighttime_efficiency_report_chunk_size', 500)),
            'timeout' => max(5, (int) config('fleet.wialon.nighttime_efficiency_report_timeout', 90)),
        ];
    }
        ...
        return $this->reportSessionLock->run(function () use ($group, $from, $to, $sid): array {
            ...
            try {
                ...
                if (! is_array($reportResult) || ! is_array($reportResult['tables'] ?? null)) {
                    throw new RuntimeException('Wialon nighttime efficiency report returned an invalid result structure.');
                }
                ...
                foreach ($reportResult['tables'] as $index => $table) {
                    ...
                    if (count($rows) !== $rowCount) {
                        throw new RuntimeException("Wialon nighttime efficiency table {$index} returned ".count($rows)." of {$rowCount} rows.");
                    }
                ...
                if ($reportResult['tables'] !== []
                    ...
                    throw new RuntimeException('Wialon nighttime efficiency report did not return the Engine hours table.');
                }
            ...
            return $response ?? throw new RuntimeException('Wialon nighttime efficiency report returned no response.');
        });
    }
    ...
    private function apiTemplate(array $settings, CarbonInterface $date, string $sid): array
    {
        ...
        foreach ($template['tbl'] ?? [] as $index => $table) {
            ...
            if ((int) ($schedule['fl'] ?? 0) !== 1
                || (int) ($schedule['f1'] ?? -1) !== 1080
                || (int) ($schedule['t1'] ?? -1) !== 1439
                || (int) ($schedule['f2'] ?? -1) !== 0
                || (int) ($schedule['t2'] ?? -1) !== 479) {
                throw new RuntimeException('Wialon nighttime efficiency table must be limited to 18:00-07:59 Asia/Baku.');
            }

            if ($offsetMinutes !== 240) {
                throw new RuntimeException('Wialon nighttime efficiency currently requires the Asia/Baku UTC+04:00 offset.');
            }

            // Wialon applies table schedules in UTC. The local 18:00-07:59 shift
            // is one continuous API window from 14:00 through 03:59 UTC.
            $template['tbl'][$index]['sch']['f1'] = 840;
            $template['tbl'][$index]['sch']['t1'] = 1439;
            $template['tbl'][$index]['sch']['f2'] = 0;
            $template['tbl'][$index]['sch']['t2'] = 239;
        }

        if (! $engineTableFound) {
            throw new RuntimeException('Wialon nighttime efficiency template has no Engine hours table.');
        }
