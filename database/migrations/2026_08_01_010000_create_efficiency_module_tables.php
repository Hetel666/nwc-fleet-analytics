...
return new class extends Migration
{
    ...
    {
        Schema::create('efficiency_sync_runs', function (Blueprint $table): void {
            $table->id();
        ...
        Schema::create('efficiency_sync_tasks', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('run_id')->constrained('efficiency_sync_runs')->cascadeOnDelete();
            $table->foreignId('project_id')->constrained()->cascadeOnDelete();
            $table->string('wialon_group_id')->nullable();
            ...
            $table->unique(['run_id', 'project_id', 'business_date'], 'eff_sync_task_run_project_date_unique');
            $table->index(['project_id', 'business_date', 'status'], 'eff_sync_task_project_date_status_idx');
        });

        Schema::create('efficiency_daily_facts', function (Blueprint $table): void {
            $table->id();
            $table->date('business_date');
            $table->foreignId('project_id')->constrained()->cascadeOnDelete();
            ...
            $table->foreignId('report_run_id')->nullable()->constrained('efficiency_sync_runs')->nullOnDelete();
            ...
            $table->index(['business_date', 'ownership', 'efficiency_status'], 'eff_daily_date_owner_status_idx');
            $table->index(['business_date', 'project_id', 'vehicle_type'], 'eff_daily_date_project_type_idx');
        });

        Schema::create('efficiency_unmatched_report_rows', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('task_id')->constrained('efficiency_sync_tasks')->cascadeOnDelete();
            $table->foreignId('project_id')->constrained()->cascadeOnDelete();
            $table->date('business_date');
            $table->string('wialon_group_id');
            ...
            $table->index(['project_id', 'business_date'], 'eff_unmatched_project_date_idx');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('efficiency_unmatched_report_rows');
        Schema::dropIfExists('efficiency_daily_facts');
        Schema::dropIfExists('efficiency_sync_tasks');
        Schema::dropIfExists('efficiency_sync_runs');
    }
};
